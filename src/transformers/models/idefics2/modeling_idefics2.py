...
class Idefics2Model(Idefics2PreTrainedModel):
    ...
    def inputs_merger(
        ...
        real_images_inds: Optional[torch.Tensor],
    ):
        """
        ...
        inputs_embeds_with_image_tokens = []

        for inputs_embeds_, special_image_token_mask_, image_hidden_states_, real_images_inds_ in zip(
            inputs_embeds, special_image_token_mask, image_hidden_states.reshape(batch_size, -1, *image_hidden_states.shape[1:]), real_images_inds.view(batch_size, -1)
        ):
            image_hidden_states_ = image_hidden_states_[real_images_inds_].reshape(-1, image_hidden_states_.shape[-1])[:special_image_token_mask_.sum(), :]
            inputs_embeds_ = inputs_embeds_.clone()
            inputs_embeds_[special_image_token_mask_] = image_hidden_states_
            inputs_embeds_with_image_tokens.append(inputs_embeds_)

        inputs_embeds_with_image_tokens = torch.stack(inputs_embeds_with_image_tokens, dim=0)

        return inputs_embeds_with_image_tokens
    ...
    def forward(
        ...
        if past_seen_tokens == 0 and inputs_embeds is not None and image_hidden_states is not None:
            inputs_embeds = self.inputs_merger(
                ...
                real_images_inds=real_images_inds,
            )
